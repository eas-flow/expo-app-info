 ...
 * on that account alone. Not fatal: the row still prints with "-", and the
 * reason goes to stderr so it stays out of stdout.
 */
export async function runPlan(client, accounts, opts, accountDisplayNames) {
  let done = 0;

  // Warnings are printed by walking `accounts` below rather than from inside
  // the task, which would order them by response arrival instead.
