...
| 変数            | 説明                                                                                       |
| --------------- | ------------------------------------------------------------------------------------------ |
| `EXPO_TOKEN`    | パーソナルアクセストークン — 必須。上記「認証」を参照                                      |
| `EXPO_API_URL`  | GraphQLエンドポイントを上書き — 非公開のローカルテスト用                                    |
| `NO_COLOR`      | 空でない値を設定すると、TTYでもANSIカラーを無効化（[no-color.org](https://no-color.org/)） |
| `FORCE_COLOR`   | `0`以外の値を設定すると、TTYでなくてもANSIカラーを強制。両方設定時は`NO_COLOR`が優先        |
...
- （デフォルト） — アカウント内のすべての Expo (EAS) アプリを、プラットフォームごとの最新のビルド試行とともに、**どのディレクトリからでも**一覧表示
- `--platform <ios|android>` — 1つのプラットフォームに絞り込む
...
- `--stats` — UTC暦月ごとの成功/エラー/キャンセルのビルド数（`--group-by app` でアプリ単位、`--month <n>` でウィンドウを拡大）
...
3. `app.byId(...).builds(offset: 0, limit: $limit, filter: { platform })` — プラットフォームごとの直近 `N` 件のビルド試行（ステータスを問わない、`--history` を指定しない限り `limit` は1）。レスポンスはクライアント側で `createdAt` の降順にソートされます

`--stats` はステップ1〜2を再利用し、ステップ3の代わりに各アプリのビルドをページングしてクライアント側で集計します。
