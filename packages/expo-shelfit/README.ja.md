...
デフォルトのアプリ一覧は、`https://api.expo.dev/graphql` に対する3つの GraphQL クエリで構成されます:
...
そのアカウントの行は、取得できなかった列を `-` にしたまま出力され、理由は stderr に出力されます。実行全体が失敗することはありません。`--stats --group-by app` の場合はより細かい単位で劣化し、あるアプリのビルド取得に失敗してもそのアプリの行だけが `-` になります。ただし**アプリ一覧そのもの**の取得に失敗したアカウントは、アプリが分からないため1行も出力されず、stderr の警告のみで報告されます。

**`--stats` の件数は EAS の請求上の使用量と一致しますか？**

必ずしも一致しません。`--stats` は請求スコープのフィールドを一切クエリせず、件数は各アプリのビルド履歴からクライアントサイドで UTC 暦月ごとに集計しています。EAS の請求は独自のサイクルで行われ暦月とは揃わないため、両者は食い違うことがあります。
...
ありません — 保証の内容は上の **認証 & インストール** の箇条書きにまとめてあります。ポリシーの詳細と脆弱性を非公開で報告する方法については [SECURITY.md](../../.github/SECURITY.md)（英語のみ）を参照してください。
