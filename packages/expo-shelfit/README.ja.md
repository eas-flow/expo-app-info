...
- **どのディレクトリからでも、作ったアプリを一覧表示したい** — （デフォルト）アカウントに紐づく全ての Expo (EAS) アプリを一覧表示
- **これまでのビルド結果を確認したい** — `--history <N>` で最新の1件だけでなく、プラットフォームごとに直近 `N` 件のビルド試行を表示
- **ビルド結果を月単位で把握したい** — `--stats` でUTC暦月ごとの成功/エラー/キャンセルのビルド数を集計（`--group-by app` でアプリ単位、`--month <n>` でウィンドウを拡大）
