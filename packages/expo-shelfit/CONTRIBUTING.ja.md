...
**このリポジトリのファイルに issue / PR 番号を書かないでください** — コメント、JSDoc、テスト名、README、ワークフロー、テンプレートのいずれにおいても同様です。番号は説明ではありません。なぜそのコードがそうなっているかを知るために読み手を別の場所へ飛ばしますが、その別の場所は変化し、クローズされ、公開パッケージの読者からは辿れなくなります。番号の代わりに、理由をその場に書いてください — 何を観測し、何を試し、どの制約に従っているのか。

**コメントは「なぜ」を書き、「何を」は書かないでください。** 読めば分かるコードにコメントは不要です。シグネチャの言い換えや制御フローの実況は書かないでください。コメントすべきなのは、コード自身が語れないことだけです — 実 API を叩いて確認した挙動、変えると静かに壊れる制約、検討して却下した代替案など。それらも1〜2文に収めてください。

コミットメッセージ・ブランチ名・PRのタイトル/本文・リリースノートは issue 番号のルールの対象外です。そこで issue にリンクするのは問題なく、むしろ推奨されます。PRを作成する前に、リポジトリルートで以下を実行してください:
