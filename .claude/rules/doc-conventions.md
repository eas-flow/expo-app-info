...
- **No file in this repo may reference a GitHub issue or PR number.** Write
  the reason in place instead. Commit messages, branch names, PR
  titles/bodies and Release notes are outside this rule.
- **Every fact has one home.** If another document already says it, link
  rather than repeat.
- **A sample output block is generated by running the CLI**, never
  hand-edited.
- **Sample data is placeholder data** — in docs *and* test fixtures.
- Before opening a PR, `git grep -nE '#[0-9]{2,4}|issues?/[0-9]+|pull/[0-9]+' -- . ':!package-lock.json'`
  must return nothing.
...
comments, JSDoc, test names, `README.md`/`README.ja.md`, `CLAUDE.md`,
`.github/**` (workflows, PR and issue templates), and `.claude/**` (rules,
skills, agents). It applies to numbers of any width, in Japanese and English
alike.
...
comment, it belongs in the relevant `README.md`, `CONTRIBUTING.md`, or a
rules file, not behind a link.
...
- **`CLAUDE.md`** — what the repo is, repo-wide commands, the branch and
  release process, and an index of the rules below. Not the rules themselves.
- **`.claude/rules/*.md`** — the conventions every change follows, and why
  each package's code is the shape it is: query shapes, rejected
