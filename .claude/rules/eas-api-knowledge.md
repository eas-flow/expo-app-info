...
| ファイル         | 責務                                                                                        |
| ---------------- | ------------------------------------------------------------------------------------------- |
| `bin/cli.mjs`    | エラーを exit code と表示メッセージに変換する唱一の場所                                     |
...
## GraphQL API の実挙動（probe で確認済みの罰）
...
- **billing-scoped フィールド**（`subscription` / `billingPeriod` / `usageMetrics`）は billing 権限のないトークンだと該当アカウントのみ GraphQL エラー。CLI 側で捕捉して「-」表示＋stderr 警告に劒化させる（run 全体は落ささない）。
